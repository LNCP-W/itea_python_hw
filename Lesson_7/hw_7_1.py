...
class StandartModel:
    ...
    def delete_one(self, table, some_id, id_value):
        data_insert_del = f"""DELETE FROM {table} WHERE {some_id} = {id_value}"""
        with conn, conn.cursor() as cursor:
            cursor.execute(data_insert_del)
...
class Order(StandartModel):
    data_insert_create = """
                ...
                RETURNING order_id
                """
    data_insert_change_desc = """UPDATE orders SET description = %s, updated_dt = %s WHERE order_id=%s"""
    data_insert_change_creator = """UPDATE orders SET creator_id = %s, updated_dt = %s WHERE order_id = %s"""
    ...
    def __init__(self, order_type, description, serial, creator_id, status='new'):
        ...
        with conn, conn.cursor() as cursor:
            cursor.execute(data_insert_create, fields)
...
class Department(StandartModel):
    ...
    def __init__(self, department_name):
        ...
        with conn, conn.cursor() as cursor:
            cursor.execute(data_insert_create, (department_name, ))

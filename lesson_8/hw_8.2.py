...
class Order(StandartMetod):
    ...
    def __str__(self):
        return f"Order id:{self.order_id} was created {self.time_create} by employee #{str(self.creator_id)}. " \
               f"Actual status is {self.status}, type: {self.order_type}, description: {self.description}, " \
               f"serial number: {self.serial}"

    def __repr__(self):
        return f"id:{self.order_id} date:{self.time_create} employee :{str(self.creator_id)}, " \
               f"status:{self.status}, type: {self.order_type}, description: {self.description}, " \
               f"serial number: {self.serial}"
...
class Department(StandartMetod):
    ...
    def __str__(self):
        return f"Department {self.name} has id: {self.department_id}"
...
class Employees(StandartMetod):
    ...
    def __str__(self):
        return f"Employee #{self.employee_id} {self.fio} worked on position {self.position} " \
               f"in department #{self.department_id}"

    def __repr__(self):
        return f"{self.employee_id} {self.fio} {self.position} in {self.department_id}"
